...
data = pd.read_csv(cwd / 'data' / 'match_output.csv', index_col=0)

def process_data(data):
    data['time'] = pd.to_datetime(data['time'])
    data['timestamp'] = data['time'].apply(lambda x: x.timestamp())
    data['end_longitude'] = data['matched_coordinates'].apply(lambda x: eval(x)[0])
    data['end_latitude'] = data['matched_coordinates'].apply(lambda x: eval(x)[1])
